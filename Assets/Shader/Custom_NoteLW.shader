Shader "Custom/NoteLW" {
	Properties {
		_Smoothness ("Smoothness", Range(0, 1)) = 1
		_NoteSize ("NoteSize", Float) = 0.25
		[Space(12)] [Toggle(ENABLE_COLOR_INSTANCING)] _EnableColorInstancing ("Color Instancing", Float) = 0
		...
		[Space(12)] [Toggle(VERTEX_DISTORTION)] _EnableVertexDistortion ("Enable Vertex Distortion", Float) = 0
	...
	SubShader {
		...
		Pass {
			...
			fout frag(v2f inp)
			{
                fout o;
                float4 tmp0;
                tmp0 = _FinalColorMul.xxxx * _SimpleColor;

...
// Type: BeatmapEditor3D.LevelEditor.MoveArcToBeatLineCommand
...
{
  public class MoveArcToBeatLineCommand : MoveBeatmapObjectToBeatLineCommand
  {
    [Inject]
    private readonly MoveArcToBeatLineSignal _signal;
    ...
    protected override (BeatmapEditorObjectId id, BeatmapObjectType type) GetDraggedData() => (this._signal.id, BeatmapObjectType.Arc);
    ...
    protected override void GatherBeatmapObject(float beat, int column, int row)
    {
      ...
      if (this._originalHeadArc != (ArcEditorData) null)
        this._changedHeadArc = ArcEditorData.CopyWithModifications(this._originalHeadArc, beat: new float?(beat), column: new int?(column), row: new int?(row));
      if (!(this._originalTailArc != (ArcEditorData) null))
        return;
      this._changedTailArc = ArcEditorData.CopyWithModifications(this._originalTailArc, tailBeat: new float?(beat), tailColumn: new int?(column), tailRow: new int?(row));
    }

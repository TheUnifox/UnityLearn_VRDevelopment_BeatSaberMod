...
// Type: BeatmapEditor3D.InputSignals.LightTranslationTrackMouseInputController
...
{
  public class LightTranslationTrackMouseInputController : 
    AbstractMouseInputController<BeatmapEditorObjectId>
  {
    [SerializeField]
    private LightTranslationEventBoxTrackView _lightTranslationEventBoxTrackView;
    [SerializeField]
    ...
    protected void OnEnable()
    {
      ...
      this.objectActions = new Dictionary<int, Action<BeatmapEditorObjectId>>()
      {
        ...
        },
        {
          ...
          new Action<BeatmapEditorObjectId>(this.HandleEventObjectTranslationUp)
        },
        {
          ...
          new Action<BeatmapEditorObjectId>(this.HandleEventObjectTranslationDown)
        }
      };
    }
      ...
      BeatmapEditorObjectId objectId)
    {
      ...
      this._lightTranslationEventBoxTrackView.eventObjects[objectId].SetHighlight(mouseInputType == MouseInputType.Enter);
    }
    ...
    private void HandleEventGridCreateObject()
    {
      if (this._beatmapState.interactionMode != InteractionMode.Place)
        return;
      this._signalBus.Fire<PlaceLightTranslationEventSignal>(new PlaceLightTranslationEventSignal(this._lightTranslationEventBoxTrackView.startBeat, this._lightTranslationEventBoxTrackView.id));
    }
    ...
    private void HandleEventObjectClick(BeatmapEditorObjectId id)
    {
      if (this._beatmapState.interactionMode != InteractionMode.Delete)
        return;
      this._signalBus.Fire<DeleteLightEventSignal>(new DeleteLightEventSignal(this._lightTranslationEventBoxTrackView.id, id));
    }

    private void HandleEventObjectSelect(BeatmapEditorObjectId id) => this._signalBus.Fire<SelectSingleEventBoxesEventSignal>(new SelectSingleEventBoxesEventSignal(this._lightTranslationEventBoxTrackView.id, id, true));

    private void HandleEventObjectDelete(BeatmapEditorObjectId lightColorEventId) => this._signalBus.Fire<DeleteLightEventSignal>(new DeleteLightEventSignal(this._lightTranslationEventBoxTrackView.id, lightColorEventId));

    private void HandleEventObjectEnter(BeatmapEditorObjectId baseEventId) => this._signalBus.Fire<ChangeHoverEventBoxBaseEventSignal>(new ChangeHoverEventBoxBaseEventSignal(this._lightTranslationEventBoxTrackView.id, baseEventId));

    private void HandleEventObjectExit(BeatmapEditorObjectId baseEventId) => this._signalBus.Fire<ChangeHoverEventBoxBaseEventSignal>(new ChangeHoverEventBoxBaseEventSignal(this._lightTranslationEventBoxTrackView.id, BeatmapEditorObjectId.invalid));

    private void HandleEventObjectTranslationUp(BeatmapEditorObjectId id) => this._signalBus.Fire<ModifyHoveredLightTranslationDeltaTranslationSignal>(new ModifyHoveredLightTranslationDeltaTranslationSignal(1f));

    private void HandleEventObjectTranslationDown(BeatmapEditorObjectId id) => this._signalBus.Fire<ModifyHoveredLightTranslationDeltaTranslationSignal>(new ModifyHoveredLightTranslationDeltaTranslationSignal(-1f));
  }
}

...
namespace BeatmapEditor3D.DataModels
{
  public class SliderEditorData : BaseEditorData, IEquatable<SliderEditorData>
  {
    ...
    {
      return SliderEditorData.CreateNewWithId(id ?? sliderData.id, type ?? sliderData.type, colorType ?? sliderData.colorType, headBeat ?? sliderData.beat, headColumn ?? sliderData.column, headRow ?? sliderData.row, cutDirection ?? sliderData.cutDirection, controlPointLengthMultiplier ?? sliderData.controlPointLengthMultiplier, tailBeat ?? sliderData.tailBeat, tailColumn ?? sliderData.tailColumn, tailRow ?? sliderData.tailRow, tailCutDirection ?? sliderData.tailCutDirection, tailControlPointLengthMultiplier ?? sliderData.tailControlPointLengthMultiplier, midAnchorMode ?? sliderData.midAnchorMode, sliceCount ?? sliderData.sliceCount, squishAmount ?? sliderData.squishAmount);
    }

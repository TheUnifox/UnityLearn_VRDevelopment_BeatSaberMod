...
// Type: BeatmapEditor3D.Commands.ChangeHoveredChainSliceCountCommand
...
{
  public class ChangeHoveredChainSliceCountCommand : 
    IBeatmapEditorCommandWithHistoryMergeable,
    IBeatmapEditorCommandWithHistory,
    IBeatmapEditorCommand
  {
    [Inject]
    private readonly ChangeHoveredChainSliceCountSignal _signal;
    [Inject]
    ...
    public void Execute()
    {
      ...
      if (this._originalChain == (ChainEditorData) null)
        return;
      this._changedChain = ChainEditorData.CopyWithModifications(this._originalChain, sliceCount: new int?(Mathf.Clamp(this._originalChain.sliceCount + (int) Mathf.Sign(this._signal.delta), 2, 100)));
    ...
    {
      return previousCommand is ChangeHoveredChainSliceCountCommand sliceCountCommand && sliceCountCommand._originalChain.id == this._originalChain.id;
    }

    public void MergeWith(
      IBeatmapEditorCommandWithHistoryMergeable previousCommand)
    {
      this._originalChain = ((ChangeHoveredChainSliceCountCommand) previousCommand)._originalChain;
    }
  }
}

// Decompiled with JetBrains decompiler
// Type: BeatmapEditor3D.Commands.EndEventsSelectionSignal
// Assembly: BeatmapEditor3D, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 1F08665C-E1B6-4752-A219-2B54516F316A
// Assembly location: C:\Program Files\Oculus\Software\Software\hyperbolic-magnetism-beat-saber\Beat Saber_Data\Managed\BeatmapEditor3D.dll

namespace BeatmapEditor3D.Commands
{
  public class EndEventsSelectionSignal
  {
    public readonly float beat;
    public readonly BasicBeatmapEventType BasicBeatmapEventType;
    public readonly bool commit;

    public EndEventsSelectionSignal()
    {
      this.beat = 0.0f;
      this.BasicBeatmapEventType = BasicBeatmapEventType.Event0;
      this.commit = false;
    }

    public EndEventsSelectionSignal(
      float beat,
      BasicBeatmapEventType BasicBeatmapEventType,
      bool commit)
    {
      this.beat = beat;
      this.BasicBeatmapEventType = BasicBeatmapEventType;
      this.commit = commit;
    }
  }
}

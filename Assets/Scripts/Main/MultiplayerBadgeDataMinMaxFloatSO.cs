...
public abstract class MultiplayerBadgeDataMinMaxFloatSO : MultiplayerBadgeDataSO
{
  [Space]
  [SerializeField]
  ...
  protected abstract float GetValue(MultiplayerPlayerResultsData result);
    ...
    float randomMultiplier)
  {
    ...
    foreach (MultiplayerPlayerResultsData result in (IEnumerable<MultiplayerPlayerResultsData>) resultsData)
    {
      ...
      else if (Mathf.Approximately(a, b))
      {
        a = b;
    ...
    float num2 = num1 / (float) resultsData.Count;
    if ((double) num2 <= 0.0)
      num2 = 1f;
    float weight = a / num2 * this._weightMultiplier * randomMultiplier;
    ...
    float randomMultiplier)
  {
    ...
    if (awardedPlayer == null)
      return (MultiplayerBadgeAwardData) null;
    double num2 = (double) num1 / (double) resultsData.Count;
    if ((double) a <= 0.0)
      a = 1f;
    double num3 = (double) a;
    float weight = (float) (num2 / num3) * this._weightMultiplier * randomMultiplier;
    string title = Localization.Get(this.titleLocalizationKey);
    string format = Localization.GetFormat(this.subtitleLocalizationKey, (object) a);
    return new MultiplayerBadgeAwardData(awardedPlayer, weight, title, format, (MultiplayerBadgeDataSO) this);
  }

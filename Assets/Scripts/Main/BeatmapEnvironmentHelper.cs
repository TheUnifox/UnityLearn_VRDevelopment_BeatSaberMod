// Decompiled with JetBrains decompiler
// Type: BeatmapEnvironmentHelper
// Assembly: Main, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 89B9E799-8342-47B3-85ED-672D6883482A
// Assembly location: C:\Program Files\Oculus\Software\Software\hyperbolic-magnetism-beat-saber\Beat Saber_Data\Managed\Main.dll

public static class BeatmapEnvironmentHelper
{
  public static EnvironmentInfoSO GetEnvironmentInfo(this IDifficultyBeatmap difficultyBeatmap)
  {
    IBeatmapLevel level = difficultyBeatmap.level;
    return !difficultyBeatmap.parentDifficultyBeatmapSet.beatmapCharacteristic.containsRotationEvents ? level.environmentInfo : level.allDirectionsEnvironmentInfo;
  }
}

// Decompiled with JetBrains decompiler
// Type: BloomPrePassBackgroundColorsGradientTintColorWithLightId
// Assembly: HMRendering, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 0C55B48F-2592-4126-9F83-BBF1ACE1B216
// Assembly location: C:\Program Files\Oculus\Software\Software\hyperbolic-magnetism-beat-saber\Beat Saber_Data\Managed\HMRendering.dll

using UnityEngine;

public class BloomPrePassBackgroundColorsGradientTintColorWithLightId : RuntimeLightWithIds
{
  [SerializeField]
  protected BloomPrePassBackgroundColorsGradient _bloomPrePassBackgroundColorsGradient;
  [SerializeField]
  protected bool _useGrayscale;
  [SerializeField]
  [DrawIf("_useGrayscale", true, DrawIfAttribute.DisablingType.DontDraw)]
  protected float grayscaleFactor;

  protected override void ColorWasSet(Color color)
  {
    if (this._useGrayscale)
      color = Color.Lerp(color, Color.white * color.maxColorComponent, Mathf.Clamp01(this.grayscaleFactor));
    this._bloomPrePassBackgroundColorsGradient.tintColor = color;
  }
}
